...
public class BlockUtils {
    ...
    static {
        try {
            ...
            m_chunkPrecipitationHeightMapField = Chunk.class
                .getDeclaredField(Environment.getRuntimeName("precipitationHeightMap", "field_76638_b"));
            m_chunkPrecipitationHeightMapField.setAccessible(true);
            m_chunkHeightMapField = Chunk.class
                .getDeclaredField(Environment.getRuntimeName("heightMap", "field_76634_f"));
            m_chunkHeightMapField.setAccessible(true);
            m_chunkStorageArraysField = Chunk.class
                .getDeclaredField(Environment.getRuntimeName("storageArrays", "field_76652_q"));
            m_chunkStorageArraysField.setAccessible(true);
            m_chunkIsModifiedField = Chunk.class
                .getDeclaredField(Environment.getRuntimeName("isModified", "field_76643_l"));
            m_chunkIsModifiedField.setAccessible(true);
            m_chunkRelightBlockMethod = Chunk.class.getDeclaredMethod(
                Environment.getRuntimeName("relightBlock", "func_76615_h"),
                int.class,
                int.class,
                int.class);
            m_chunkRelightBlockMethod.setAccessible(true);
            m_chunkPropagateSkylightOcclusionMethod = Chunk.class.getDeclaredMethod(
                Environment.getRuntimeName("propagateSkylightOcclusion", "func_76595_e"),
                int.class,
                int.class);
    ...
    public static BlockSet searchForBlocks(int x, int y, int z, int maxNumBlocks, BlockExplorer explorer,
        Neighbors neighbors) {
        return searchForBlocks(new Coords(x, y, z), maxNumBlocks, explorer, neighbors);
    }

    public static BlockSet searchForBlocks(final Coords source, int maxNumBlocks, BlockExplorer explorer,
        Neighbors neighbors) {
    ...
    public static Boolean searchForCondition(int x, int y, int z, int maxNumBlocks, BlockConditionChecker checker,
        BlockExplorer explorer, Neighbors neighbors) {
        return searchForCondition(new Coords(x, y, z), maxNumBlocks, checker, explorer, neighbors);
    }

    public static Boolean searchForCondition(Coords source, int maxNumBlocks, final BlockConditionChecker checker,
        BlockExplorer explorer, Neighbors neighbors) {
        return searchForBlock(source, maxNumBlocks, checker, explorer, neighbors) != null;
    }

    public static Coords searchForBlock(int x, int y, int z, int maxNumBlocks, BlockConditionChecker checker,
        BlockExplorer explorer, Neighbors neighbors) {
        return searchForBlock(new Coords(x, y, z), maxNumBlocks, checker, explorer, neighbors);
    }

    public static Coords searchForBlock(final Coords source, int maxNumBlocks, final BlockConditionChecker checker,
        BlockExplorer explorer, Neighbors neighbors) {
    ...
    public static void exploreBlocks(Coords source, int maxNumBlocks, BlockCallback callback, BlockExplorer explorer,
        Neighbors neighbors) {
    ...
    public static List<BlockSet> getConnectedComponents(BlockSet blocks, Neighbors neighbors) {
        ...
        while (!remainingBlocks.isEmpty()) {
            // get a block
            Coords coords = remainingBlocks.iterator()
                .next();

            // do BFS from this block to find the connected component
            BlockSet component = new BlockSet(
                BlockUtils.searchForBlocks(coords, remainingBlocks.size(), new BlockExplorer() {
    ...
    public static BlockSet getHoleFromInnerBoundary(BlockSet innerBoundary, final BlockSet blocks,
        Neighbors neighbors) {
    ...
    public static BlockSet getHoleFromInnerBoundary(BlockSet innerBoundary, final BlockSet blocks, Neighbors neighbors,
        ...
        Coords sourceBlock = innerBoundary.iterator()
            .next();
        BlockSet holeBlocks = BlockUtils.searchForBlocks(sourceBlock, volume, new BlockExplorer() {

            @Override
            public boolean shouldExploreBlock(Coords coords) {
                return !blocks.contains(coords) && (yMin == null || coords.y >= yMin)
                    && (yMax == null || coords.y <= yMax);
            }
        }, neighbors);
    ...
    public static boolean changeBlockWithoutNotifyingIt(World world, int x, int y, int z, Block targetBlock,
        int targetBlockMeta) {
        return changeBlockWithoutNotifyingIt(
            world,
            x,
            y,
            z,
            targetBlock,
            targetBlockMeta,
            UpdateRules.UpdateNeighborsAndClients);
    }

    public static boolean changeBlockWithoutNotifyingIt(World world, int x, int y, int z, Block targetBlock,
        int targetBlockMeta, UpdateRules updateRules) {
        ...
        try {
            ...
            if (oldBlock == targetBlock) {
                if (oldBlockMeta != targetBlockMeta) {
                    Log.logger.warning(
                        String.format(
                            "Ignoring block metadata change for block %s at (%d,%d,%d)",
                            oldBlock.getUnlocalizedName(),
                            x,
                            y,
                            z));
                }
