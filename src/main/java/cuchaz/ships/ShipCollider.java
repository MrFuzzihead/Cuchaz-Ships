...
public class ShipCollider {
    ...
    public void onNearbyEntityMoved(double oldX, double oldY, double oldZ, double oldYSize, Entity entity) {
        ...
        // to make collisions for standing on blocks more robust, if the old box is JUST beneath the top of a block, pop
        // it up.
        ...
        if (isPlayerCrouching) {
            // for walk-over prevention, move the query box just a little bit farther out to avoid precision/roundoff
            // problems
            ...
            while (dx != 0 && m_ship.getShipWorld()
                .getGeometry()
                .rangeQuery(oldEntityBox.getOffsetBoundingBox(dx + bufferX, -1.0, 0.0))
                .isEmpty()) {
                dx = stepTowardsZero(dx, StepSize);
            }
            while (dz != 0 && m_ship.getShipWorld()
                .getGeometry()
                .rangeQuery(oldEntityBox.getOffsetBoundingBox(0.0, -1.0, dz + bufferZ))
                .isEmpty()) {
            ...
            while (dx != 0 && dz != 0
                && m_ship.getShipWorld()
                    .getGeometry()
                    .rangeQuery(oldEntityBox.getOffsetBoundingBox(dx + bufferX, -1.0, dz + bufferZ))
                    .isEmpty()) {
                dx = stepTowardsZero(dx, StepSize);
                dz = stepTowardsZero(dz, StepSize);
            }
        ...
        ((IMixinEntity) entity).callUpdateFallState(dy, entity.onGround);
    }

    public AxisAlignedBB getBlockBoxInBlockSpace(Coords coords) {
        Block block = m_ship.getShipWorld()
            .getBlock(coords);
    ...
    public AxisAlignedBB getBlockWorldBoundingBox(AxisAlignedBB box, Coords coords, double shipX, double shipY,
        double shipZ, float shipYaw) {
        return getBlockBoundingBox(box, coords, shipX, shipY, shipZ, shipYaw, null);
    }

    public AxisAlignedBB getBlockBoundingBox(AxisAlignedBB box, Coords coords, double shipX, double shipY, double shipZ,
        float shipYaw, EntityShip ship) {
    ...
    public void moveShip(double dx, double dy, double dz, float dYaw) {
        ...
        for (Coords coords : m_ship.getShipWorld()
            .coords()) {
    ...
    public boolean isEntityStandingOnBlock(Entity entity) {
        ...
        for (Coords coords : m_ship.getShipWorld()
            .getGeometry()
            .rangeQuery(checkBox.expand(0, 1, 0))) {
    ...
    public boolean isEntityOnLadder(EntityLivingBase entity) {
        ...
        for (Coords coords : shipWorld.getGeometry()
            .rangeQuery(entityBox)) {
    ...
    public List<MovingObjectPosition> lineSegmentQuery(final Vec3 from, Vec3 to) {
        ...
        BlockSet nearbyBlocks = m_ship.getShipWorld()
            .getGeometry()
            .rangeQuery(box);
        ...
        for (Coords coords : nearbyBlocks) {
            ...
            Block block = m_ship.getShipWorld()
                .getBlock(coords);
            MovingObjectPosition intersection = block
                .collisionRayTrace(m_ship.getShipWorld(), coords.x, coords.y, coords.z, from, to);
    ...
    public void getIntersectingWorldBlocks(BlockSet worldBlocks, BlockSet shipBlocks, double epsilon,
        boolean includeAir) {
    ...
    private void checkBlockCollision(CollisionResult result, Coords coords, double dx, double dy, double dz,
        float dYaw) {
        ...
        for (Coords worldCoords : nearbyWorldBlocks) {
            ...
            for (AxisAlignedBB worldBlockBox : worldBlockBoxes) {
                blockScaling = Math
                    .min(blockScaling, getScalingToAvoidCollision(shipBlockBox, dx, dy, dz, worldBlockBox));
            }
    ...
    private void checkShipCollision(CollisionResult result, Coords coords, double dx, double dy, double dz, float dYaw,
        EntityShip ship) {
    ...
    private double getScalingToAvoidCollision(AxisAlignedBB box, double dx, double dy, double dz,
    ...
    private double getScalingToAvoidCollision(double delta, double selfMin, double selfMax, double obstacleMin,
        double obstacleMax) {
    ...
    private List<PossibleCollision> trajectoryQuery(AxisAlignedBB oldBox, AxisAlignedBB newBox) {
        ...
        if (m_ship != null && m_ship.getShipWorld() != null
            && m_ship.getShipWorld()
                .getGeometry() != null)
        // NOTE: if one of these things is null, the ship probably hasn't loaded yet, so there shouldn't be any
        // collisions
        {
            for (Coords coords : m_ship.getShipWorld()
                .getGeometry()
                .rangeQuery(trajectoryBox.expand(1, 1, 1)))
            // NOTE: expand trajectoryBox by 1 so we pick up boxes whose collision boxes are outside their bounding
            // boxes
            {
                boxes.clear();
