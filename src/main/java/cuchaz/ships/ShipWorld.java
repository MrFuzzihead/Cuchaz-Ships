...
public class ShipWorld extends DetachedWorld {
    ...
    public ShipWorld(World world, BlocksStorage storage, BlockMap<TileEntity> tileEntities,
        BlockMap<EntityHanging> hangingEntities, int biomeId) {
        this(world);
    ...
    public void restoreToWorld(World world, Map<Coords, Coords> correspondence, int waterHeightInBlockSpace) {
        ...
        for (Coords coordsShip : getDisplacement().getTrappedAirFromWaterHeight(waterHeightInBlockSpace)) {
            ...
            if (BlockProperties.isWater(block) || block == Ships.m_blockAirWall) {
                BlockUtils.removeBlockWithoutNotifyingIt(
                    world,
                    coordsWorld.x,
                    coordsWorld.y,
                    coordsWorld.z,
                    UpdateRules.UpdateClients);
            }
        }
    ...
    public BlockMap<EntityHanging> getNearbyHangingEntities(World world, BlockSet blocks) {
        ...
        List<EntityHanging> hangingEntities = (List<EntityHanging>) world
            .getEntitiesWithinAABB(EntityHanging.class, box);
    ...
    public void updateEntities() {
        ...
        Iterator<Map.Entry<Coords, TileEntity>> iter = m_tileEntities.entrySet()
            .iterator();
        while (iter.hasNext()) {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            if (block != Blocks.air) {
                ...
                try {
                    ...
                    Ships.logger
                        .error(t, "Block threw up during random tick: %s", Block.blockRegistry.getNameForObject(block));
                }
            }
        }
    }
    ...
    @Override
    public void spawnParticle(String name, double x, double y, double z, double motionX, double motionY,
        double motionZ) {
        if (m_ship == null) {
            return;
        }
