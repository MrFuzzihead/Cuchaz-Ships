...
public class BlockShip extends Block {
    ...
    public IIcon getIcon(int side, int meta) {
        switch (BlockSide.getById(side)) {
            ...
            case East:
            case West:
                return ShipType.getByMeta(meta)
                    .getIcon();
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float xOffset,
        ...
        if (world instanceof ShipWorld) {
            ...
            boolean isPaddleEquipped = player.getCurrentEquippedItem() != null && player.getCurrentEquippedItem()
                .getItem() == Ships.m_itemPaddle;
            ShipWorld shipWorld = (ShipWorld) world;
            if (isPaddleEquipped && shipWorld.getShipType()
                .isPaddleable()
                && shipWorld.getShip()
                    .getCollider()
                    .isEntityAboard(player)) {
