    ...
    V1(1) {
                ...
                if (storage.block == null) {
                    ...
                    if (!warnedBlocks.contains(blockName)) {
                        Ships.logger.warning(
                            "Can't find block %s on this server! Block has been replaced with wood planks",
                            blockName);
        ...
        protected void onWrite(BlocksStorage blocks, DataOutputStream out) throws IOException {
            ...
            for (Coords coords : blocks.coords()) {
                ...
                out.writeShort(
                    Bits.packUnsigned(Block.getIdFromBlock(block.block), 12, 0) | Bits.packUnsigned(block.meta, 4, 12));
            }
        }
    };
