...
public class RenderShip2D {
    ...
    public static void drawWater(BlockArray envelope, Double waterHeight, Integer sinkHeight, int x, int y, double z,
        ...
        if (waterHeight != null && sinkHeight != null) {
            ...
            fontRenderer.drawString(
                GuiString.Sink.getLocalizedText(),
                x,
                y + maxHeight - sinkLineY - fontRenderer.FONT_HEIGHT,
                SinkColor);
    ...
    public static void drawShip(BlockArray envelope, BlockSide side, World world, int x, int y, double z, int maxWidth,
        int maxHeight) {
        double blockSize = getBlockSize(maxWidth, maxHeight, envelope);
    ...
    public static void drawShipAsColor(BlockArray envelope, int color, int x, int y, double z, int maxWidth,
        int maxHeight) {
        double blockSize = getBlockSize(maxWidth, maxHeight, envelope);
        double shipWidth = (double) envelope.getWidth() * blockSize;
        double shipHeight = (double) envelope.getHeight() * blockSize;
        ...
        for (int u = envelope.getUMin(); u <= envelope.getUMax(); u++) {
            for (int v = envelope.getVMin(); v <= envelope.getVMax(); v++) {
                ...
                drawColoredBlock(
                    ...
                    (maxHeight - shipHeight) / 2.0 + y
                        + (envelope.getHeight() - envelope.toZeroBasedV(v) - 1) * blockSize,
                    z,
                    blockSize,
                    blockSize,
                    color);
    ...
    private static double getBlockSize(int maxWidth, int maxHeight, BlockArray envelope) {
        double blockSize = Math
            .min((double) maxWidth / (double) envelope.getWidth(), (double) maxHeight / (double) envelope.getHeight());
