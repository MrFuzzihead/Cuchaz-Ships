...
public class RenderShip extends Render {
    ...
    public void doRender(EntityShip ship, double x, double y, double z, float yaw, float partialTickTime) {
        ...
        for (EntityHanging hangingEntity : shipWorld.hangingEntities()
            .values()) {
        ...
        for (Coords coords : shipWorld.coords()) {
            ...
            if (tileEntity != null && TileEntityRendererDispatcher.instance.hasSpecialRenderer(tileEntity)) {
                ...
                try {
                    ...
                    Ships.logger.error(
                        t,
                        "Tile entity threw up while rendering: %s",
                        Block.blockRegistry.getNameForObject(tileEntity.getBlockType()));
                }
            }
        }
        ...
        for (Coords coords : shipWorld.coords()) {
            ...
            try {
                ...
                Ships.logger.warning(
                    t,
                    "Block: %s couldn't render properly! Blocks of this type will not be rendered again.",
    ...
    public static void renderBlock(RenderBlocks renderBlocks, ShipWorld shipWorld, Block block, Coords coords) {
        ...
        if (block.getRenderType() == 0) {
            ...
            renderBlocks
                .renderStandardBlockWithColorMultiplier(block, coords.x, coords.y, coords.z, colorR, colorG, colorB);
        } else {
    ...
    private void renderDebug(EntityShip ship, double x, double y, double z, float yaw) {
        ShipDebugRenderInfo info = ship.getCollider()
            .getDebugRenderInfo();
        ...
        for (Coords coords : ship.getShipWorld()
            .coords()) {
            boxes.clear();
            ship.getCollider()
                .getCollisionBoxesInBlockSpace(
                    boxes,
                    coords,
                    ship.getCollider()
                        .getBlockBoxInBlockSpace(coords));
            int color;
            if (info.getCollidedCoords()
                .contains(coords)) {
        ...
        for (Coords coords : ship.getShipWorld()
            .getDisplacement()
            .getTrappedAirFromWaterHeight(waterHeightInBlockSpace)) {
            AxisAlignedBB box = ship.getCollider()
                .getBlockBoxInBlockSpace(coords);
        ...
        // where is the camera?
        Vec3 camera = Vec3
            .createVectorHelper(RenderManager.renderPosX, RenderManager.renderPosY, RenderManager.renderPosZ);
