...
public class GuiShipPropulsion extends GuiShip {
    ...
    public GuiShipPropulsion(final World world, int helmX, int helmY, int helmZ) {
        ...
        m_topLinearSpeed = 0;
        m_topAngularSpeed = 0;
        if (m_shipLauncher.getShipPhysics()
            .willItFloat()) {
            m_topLinearSpeed = m_shipLauncher.getShipPhysics()
                .simulateLinearAcceleration(m_propulsion).topSpeed;
            m_topAngularSpeed = m_shipLauncher.getShipPhysics()
                .simulateAngularAcceleration(m_propulsion).topSpeed;
        }
    ...
    protected void drawForeground(int mouseX, int mouseY, float partialTickTime) {
        ...
        if (m_shipLauncher == null) {
            ...
            drawLabelValueText(
                ...
                String.format(
                    "%.1f tonnes",
                    m_shipLauncher.getShipPhysics()
                        .getMass()),
                1);
            drawLabelValueText(
                "Thrust",
                String.format("%.1f KN", Util.perTick2ToPerSecond2(m_propulsion.getTotalThrust(0))),
                2);
            drawLabelValueText("Top Speed", String.format("%.1f m/s", Util.perTickToPerSecond(m_topLinearSpeed)), 3);
            drawLabelValueText(
                "Turning Speed",
                String.format("%.1f deg/s", Util.perTickToPerSecond(m_topAngularSpeed)),
                4);
    ...
    private void drawPropulsion() {
        ...
        if (ShaderLoader.areShadersSupported()) {
            ...
            RenderShip2D
                .drawShip(m_shipEnvelope, BlockSide.Top, m_shipLauncher.getShipWorld(), x, y, zLevel, width, height);
            GL20.glUseProgram(0);
        } else {
        ...
        RenderShip2D
            .drawShip(m_propulsionEnvelope, BlockSide.Top, m_shipLauncher.getShipWorld(), x, y, zLevel, width, height);
        RenderShip2D
            .drawShip(m_helmEnvelope, BlockSide.Top, m_shipLauncher.getShipWorld(), x, y, zLevel, width, height);
    }
}
