...
public class ShipPhysics {
    ...
    public ShipPhysics(BlocksStorage blocks) {
        ...
        m_sinkWaterHeight = m_blocks.getDisplacement()
            .getLastFillY();

        // is the ship unsinkable?
        ScaledDisplacementEntry lastDisplacement = getScaledDisplacement(
            m_blocks.getDisplacement()
                .getMaxY() + 1);
    ...
    public double getDisplacedWaterMass(double waterHeight) {
        ...
        return ((double) getUnderwaterDisplacement(surfaceLevel)
            + (double) getSurfaceDisplacement(surfaceLevel) * surfaceFraction) * getWaterBlockMass();
    }
    ...
    public double getLinearAccelerationDueToDrag(Vec3 velocity, double waterHeight) {
        ...
        for (Coords coords : m_blocks.getGeometry()
            .getEnvelopes()
            .getEnvelope(leadingSide)
            .toBlockSet()) {
    ...
    public float getAngularAccelerationDueToDrag(float motionYaw, double waterHeight) {
        ...
        double angularViscosity = 0 + getAngularViscosity(BlockSide.North, waterHeight, m_centerOfMass.xCoord)
    ...
    public List<AccelerationEntry> getLinearAcceleration(Propulsion propulsion, double stopSpeed, int numSteps) {
        ...
        for (int i = 0; i < numSteps; i++) {
            ...
            velocity.xCoord = speed * propulsion.getFrontSide()
                .getDx();
            velocity.zCoord = speed * propulsion.getFrontSide()
                .getDz();
        ...
        for (Coords coords : m_blocks.coords()) {
            ...
            buf.append(
                String.format(
                    ...
                    coords.y,
                    coords.z,
                    m_blocks.getBlock(coords).block,
                    mass));
        }
        return buf.toString();
    }
    ...
    private Double computeEquilibriumWaterHeight() {
        ...
        for (int y = minY; y <= maxY + 1; y++) {
            ...
            if (displacedWaterMass > m_shipMass) {
                ...
                return y + (m_shipMass - underwaterDisplacement * getWaterBlockMass()) / surfaceDisplacement
                    / getWaterBlockMass();
            }
        }
