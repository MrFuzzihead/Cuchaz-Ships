...
public class PacketPlaceProjector extends Packet<PacketPlaceProjector> {
    ...
    public PacketPlaceProjector(String encodedBlocks, int x, int y, int z) {
        ...
        if (m_encodedBlocks.length > PacketPasteShip.MaxSize) {
            ...
            throw new IllegalArgumentException(
                "Ship description size exceeds " + PacketPasteShip.MaxSize
                    + " bytes. If this is a legitimate use, we need a bigger size");
        }
        m_x = x;
        m_y = y;
        m_z = z;
    ...
    private IMessage onReceivedServer(NetHandlerPlayServer netServer) {
        ...
        // place the projector
        try {
            ShipWorld shipWorld = ShipClipboard
                .createShipWorld(netServer.playerEntity.worldObj, Base64.encodeBase64String(m_encodedBlocks));
