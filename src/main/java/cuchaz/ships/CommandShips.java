...
public class CommandShips extends CommandBase {
                    ...
                    BlockUtils.exploreBlocks(coords, NumBlocks, new BlockCallback() {
                        ...
                        public SearchAction foundBlock(Coords coords) {
                            Block block = sender.getEntityWorld()
                                .getBlock(coords.x, coords.y, coords.z);
                ...
                for (Coords coords : airWallBlocks) {
                    ...
                    if (block == Ships.m_blockAirWall) {
                        ...
                        sender.getEntityWorld()
                            .setBlock(coords.x, coords.y, coords.z, Blocks.water);
                    } else if (block == Ships.m_blockAirRoof) {
                        sender.getEntityWorld()
                            .setBlock(coords.x, coords.y, coords.z, Blocks.air);
                    }
                }
            ...
            public void process(ICommandSender sender, String[] args) {
                ...
                if (args.length <= 0) {
                    ...
                    if (block == Blocks.air && !blockId.equals(Block.blockRegistry.getNameForObject(Blocks.air))) {
                        ...
                        buf.append(
                            String.format(
                                "%-20s Ovr:%s Mass:%.1f Vol:%.1f WT:%s Ignrd:%s Water:%s",
    ...
    private static void replyAllAdmins(ICommandSender sender, String msg) {
        for (EntityPlayerMP player : (List<EntityPlayerMP>) MinecraftServer.getServer()
            ...
            boolean isPlayerOpped = MinecraftServer.getServer()
                .getConfigurationManager()
                .func_152603_m()
                .func_152700_a(player.getCommandSenderName()) != null;
            if (isPlayerOpped) {
