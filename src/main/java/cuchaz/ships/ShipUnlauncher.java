...
public class ShipUnlauncher {
    ...
    public static enum UnlaunchFlag {
        ...
        TouchingOnlySeparatorBlocks(true) {
            ...
            @Override
            public boolean computeValue(ShipUnlauncher unlauncher) {
                // if any placed ship block has a neighbor that's not a ship block and not a separator block, the flag
                // fails
    ...
    private void computeCorrespondence() {
        ...
        allCoords.addAll(
            m_ship.getShipWorld()
                .getDisplacement()
                .getTrappedAirFromWaterHeight(m_waterHeightInBlockSpace));
    ...
    public void unlaunch() {
        ...
        m_ship.getShipWorld()
            .restoreToWorld(m_ship.worldObj, m_correspondence, m_waterHeightInBlockSpace);
