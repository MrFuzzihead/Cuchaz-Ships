...
public class ShipLauncher {
    ...
    public static enum LaunchFlag {
        ...
        RightNumberOfBlocks {
            ...
            public boolean computeValue(ShipLauncher launcher) {
                return launcher.m_blocks != null && !launcher.m_blocks.isEmpty()
                    && launcher.m_blocks.size() <= launcher.m_shipType.getMaxNumBlocks() + 1; // +1 since the ship block
                                                                                              // is free
            }
        },
        WillItFloat {
    ...
    public ShipLauncher(final World world, Coords shipBlock) {
        ...
        m_blocks = BlockUtils
            .searchForBlocks(m_shipBlock.x, m_shipBlock.y, m_shipBlock.z, numBlocksToCheck, new BlockExplorer() {
    ...
    public BoundingBoxInt getShipBoundingBox() {
        ...
        return m_shipWorld.getGeometry()
            .getEnvelopes()
            .getBoundingBox();
    }
    ...
    public static void removeShipFromWorld(World world, ShipWorld shipWorld, Coords shipBlock,
        UpdateRules updateRules) {
        // translate the ship blocks into world space
        BlockSet worldBlocks = new BlockSet();
        for (Coords blockCoords : shipWorld.coords()) {
            worldBlocks
                .add(new Coords(blockCoords.x + shipBlock.x, blockCoords.y + shipBlock.y, blockCoords.z + shipBlock.z));
        }
        ...
        for (Coords blockCoords : shipWorld.getDisplacement()
            .getTrappedAirFromWaterHeight(waterHeight - shipBlock.y)) {
            worldCoords.x = blockCoords.x + shipBlock.x;
            worldCoords.y = blockCoords.y + shipBlock.y;
            worldCoords.z = blockCoords.z + shipBlock.z;
            BlockUtils.changeBlockWithoutNotifyingIt(
                world,
                worldCoords.x,
                worldCoords.y,
                worldCoords.z,
        ...
        for (Map.Entry<Coords, EntityHanging> entry : shipWorld.getNearbyHangingEntities(world, worldBlocks)
            .entrySet()) {
    ...
    private static Coords computeWaterBlock(World world, ShipWorld shipWorld, Coords shipBlock) {
        ...
        Envelopes envelopes = shipWorld.getGeometry()
            .getEnvelopes();
