...
public class ItemShipEraser extends Item {
    ...
    @Override
    public boolean onItemUseFirst(ItemStack itemStack, EntityPlayer player, final World world, int blockX, int blockY,
        int blockZ, int side, float hitX, float hitY, float hitZ) {
    ...
    public static boolean eraseShip(final World world, EntityPlayer player, int blockX, int blockY, int blockZ) {
        ...
        BlockSet blocks = BlockUtils
            .searchForBlocks(blockX, blockY, blockZ, shipType.getMaxNumBlocks(), new BlockExplorer() {
        ...
        if (Environment.isClient()) {
            Ships.net.getDispatch()
                .sendToServer(new PacketEraseShip(blockX, blockY, blockZ));
        } else {
            ...
            // remove the ship
            ShipLauncher.removeShipFromWorld(
                world,
                new ShipWorld(world, shipCoords, blocks),
                shipCoords,
