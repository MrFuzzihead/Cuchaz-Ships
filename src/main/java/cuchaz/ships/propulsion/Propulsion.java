...
public class Propulsion {
    ...
    public Propulsion(BlocksStorage blocksStorage) {
        ...
        if (m_blocksStorage.getShipType()
            .isPaddleable()) {
            // use a paddle!
            ...
            Ships.logger.warning(
                "Non-paddleable ship doesn't have a helm! This wasn't supposed to happen, and will probably cause problems later!");
        }
    ...
    public BlockArray getEnevelope() {
        BlockArray envelope = m_blocksStorage.getGeometry()
            .getEnvelopes()
            .getEnvelope(BlockSide.Top)
            .newEmptyCopy();
        for (PropulsionMethod method : m_methods) {
