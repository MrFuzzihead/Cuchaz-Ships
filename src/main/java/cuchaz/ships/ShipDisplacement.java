...
public class ShipDisplacement {
    ...
    public Integer getLastFillY() {
        for (Map.Entry<Integer, DisplacementEntry> entry : m_displacement.descendingMap()
            .entrySet()) {
    ...
    private void computeBoundaryAndHoles() {
        ...
        for (BlockSet component : BlockUtils.getConnectedComponents(boundaryBlocks, VoidBlockNeighbors)) {
            ...
            if (BlockUtils.isConnectedToShell(
                component.iterator()
                    .next(),
                m_blocks,
                VoidBlockNeighbors)) {
        ...
        for (int y = minY; y <= maxY + 1; y++) {
            ...
            if (blocksAtY != null) {
                ...
                for (BlockSet ySegment : ySegments) {
                    List<ClassifiedSegment> connectedSegments = getYConnectedSegments(
                        ySegment,
                        boundarySegments,
                        VoidBlockNeighbors);
                    if (connectedSegments.isEmpty()) {
                        ...
                        classifiedSegment.isTrapped = !BlockUtils.isConnectedToShell(
                            ySegment.iterator()
                                .next(),
                            m_blocks,
                            VoidBlockNeighbors,
                        ...
                        if (baseSegment.isTrapped) {
                            ...
                            ySegment.addAll(
                                BlockUtils.getHoleFromInnerBoundary(ySegment, m_blocks, VoidBlockNeighbors, y, y));
                        }
    ...
    private List<ClassifiedSegment> getYConnectedSegments(BlockSet ySegment, Iterable<ClassifiedSegment> segments,
        Neighbors neighbors) {
