...
public class EntityShip extends Entity {
    ...
    public void setDead() {
        ...
        if (!this.worldObj.isRemote) {
            ...
            List<EntityPlayer> players = (List<EntityPlayer>) this.worldObj
                .getEntitiesWithinAABB(EntityPlayer.class, queryBox);
    ...
    public void onUpdate() {
        ...
        if (m_shipWorld == null) {
            ...
            if (packet != null) {
                ...
                // ask for blocks
                Ships.net.getDispatch()
                    .sendToServer(new PacketRequestShipBlocks(getEntityId()));
            }
        }
        ...
        if (m_hasInfoFromServer) {
            ...
            double yawDelta = CircleRange.newByShortSegment(yawRadClient, yawRadServer)
                .getLength();
    ...
    public double getWaterHeight() {
        ...
        double sum = 0;
        BlockSet topEnvelope = new Envelopes(waterCoords).getEnvelope(BlockSide.Top)
            .toBlockSet();
    ...
    public RotatedBB worldToBlocks(AxisAlignedBB box) {
        ...
        Vec3 center = Vec3
            .createVectorHelper((box.minX + box.maxX) / 2, (box.minY + box.maxY) / 2, (box.minZ + box.maxZ) / 2);
        worldToShip(center);
        shipToBlocks(center);
    ...
    private void adjustMotionDueToThrustAndDrag(double waterHeightInBlockSpace) {
        ...
        if (m_sideShipForward != null) {
            if (m_sendPilotChangesToServer) {
                ...
                PacketPilotShip packet = new PacketPilotShip(
                    getEntityId(),
                    m_pilotActions,
                    m_sideShipForward,
                    linearThrottle,
            ...
            double linearAccelerationDueToThrust = m_physics.getLinearAccelerationDueToThrust(m_propulsion, speed)
                * linearThrottle
                / LinearThrottleMax;
        ...
        double linearAccelerationDueToDrag = m_physics
            .getLinearAccelerationDueToDrag(velocityInBlockCoords, waterHeightInBlockSpace);
        ...
        double angularAccelerationDueToThrust = m_physics.getAngularAccelerationDueToThrust(m_propulsion)
            * angularThrottle
            / AngularThrottleMax;
        double angularAccelerationDueToDrag = m_physics
            .getAngularAccelerationDueToDrag(motionYaw, waterHeightInBlockSpace);

        // make sure drag acceleration doesn't reverse the velocity!
        angularAccelerationDueToDrag = Math
            .min(Math.abs(motionYaw + angularAccelerationDueToThrust), angularAccelerationDueToDrag);
